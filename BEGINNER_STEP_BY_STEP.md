...
## 12) 新要求：LLM + PandasAI 怎么用（可选）

如果老师要求体现 `LLM + PandasAI`，你可以这样做：

1. 本脚本已内置以下函数：
   - `call_ollama`（调用本地开源 LLM）
   - `pandasai_audit`（PandasAI 数据质量审计）
   - `llm_evaluate_rules`（让 LLM 评价规则）
...
4. 看结果文件：
   - `outputs/summary_observations.md` 里有
     - LLM cleaning note
     - PandasAI audit note
     - LLM-based Rule Evaluation

> 没装 pandas/pandasai 或没启动 Ollama 也不会崩：脚本会自动回退到标准流程并写明原因。


---

## 13) 新增要求：mlxtend 对比 + 过滤少于2项事务

- 现在代码清洗阶段会过滤掉 item 数量少于 2 的事务（`len(basket) >= 2`）。
- 代码也提供了 `mlxtend` 对照分析（`apriori` / `fpgrowth` / `association_rules`），用于和手写算法比较。
- 如果你本机安装了 `pandas + mlxtend`，运行后会在 `summary_observations.md` 看到比较结果。
- 若未安装，脚本不会报错，会显示 skip 原因。
