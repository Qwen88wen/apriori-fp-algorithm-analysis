...
## 附录 D：LLM + PandasAI（按新要求替换的代码点）

本次已将代码按“Data cleaning and evaluation using LLM & PandasAI”要求扩展：

1. **LLM 清洗（Ollama）**
   - 新增 `call_ollama`、`llm_classify_presence`、`load_llm_value_map`。
   - 作用：对“非标准 token（既非 Y/N，也非空值）”做存在性判别，辅助清洗。

2. **PandasAI 评估**
   - 新增 `pandasai_audit`。
   - 作用：当环境安装 `pandas + pandasai` 时，自动做缺失值与质量风险分析；未安装时自动回退并记录原因。

3. **LLM 规则评估**
   - 新增 `llm_evaluate_rules`。
   - 作用：基于 support/confidence/lift 对 top 规则做自然语言评估（强规则、风险、建议）。

4. **报告输出整合**
   - `summary_observations.md` 现包含：LLM 清洗说明、PandasAI 审计说明、LLM 规则评估结果。

> 说明：关联分析核心（Apriori / FP-Growth）仍保留标准库实现，符合“可使用 standard library 做 association analysis”的要求。
...
1. **Data Cleaning**
   - 缺失/脏值处理：已做规则化清洗，并加入 LLM 判别非标准 token。
   - 删除 item<2 的事务：已实现（`len(basket) >= 2` 才保留）。
   - 数据结构转换：已转换为事务列表（`List[FrozenSet[str]]`）。

2. **mlxtend 关联分析包**
   - 已在代码加入 `mlxtend` 分析路径（apriori/fpgrowth/association_rules）与手写算法对比逻辑。
   - 若运行环境无 `pandas/mlxtend`，自动回退并在输出中注明 skip 原因。

3. **输出微调 / 可视化改进**
   - 已新增“手写 vs mlxtend 对比”与“序列模式挖掘自探索”输出段落到总结报告。

4. **LLM + PandasAI**
   - 已保留并强化：LLM 清洗、PandasAI 审计、LLM 规则评估。

5. **手写 Apriori 与 off-the-shelf 比较**
   - 代码已具备比较通道；当前环境依赖受限时会输出 skip 注记。

6. **Sequential Pattern Mining（自探索）**
   - 已新增 PrefixSpan 可选路径；未安装时输出 skip 注记。
